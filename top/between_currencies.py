...
class BetweenCurrencies:
        ...
        for index, currency in enumerate(self.all_currencies):
            ...
            for index2, currency2 in enumerate(self.all_currencies[index + 1:]):
                correlation = self.currency_handler.get_currency(currency,
                                                                 date_limit=self.start_date).get_absolute_correlation(
                    self.attribute,
    ...
    def get_correlation_plot(self, fig=None, ax=None, multiple=False, legend_name=""):
        ...
        as_list = list(filter(lambda x: numpy.isfinite(x[3]) and x[3] < 0.1, self.as_list))
        ...
        if not multiple:
            series2.hist(ax=ax, color="C0", label=legend_name + " (only if significant at 10%: " + str(
                len(series2)) + " of " + str(len(self.as_list)) + ")", bins=numpy.linspace(-1, 1, num=20)).plot(
                figure=fig)
        else:
    ...
    def get_correlation_positive_section_data(self):
        significant_correlations = list(filter(lambda x: numpy.isfinite(x[3]) and x[3] < 0.1, self.as_list))
        all_correlations = list(map(lambda x: x[2], significant_correlations))
