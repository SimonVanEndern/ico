...
class TotalCalculator:
    ...
    def __init__(self):
        ...
        fig, ax = plt.subplots()
        df = pandas.DataFrame({"Market capitalization": list(market_cap)}, index=dates)
        ax.set(xlabel="Time", ylabel="Market Capitalization in USD")
        ...
        df2 = pandas.DataFrame({"Volume": list(volume)}, index=dates)
        ax2.set(xlabel="Time", ylabel="Volume in USD")
